...
> **Amended by [ADR-0014](0014-durable-version-groups-and-excluded-entries.md).** The deferred version-link and "omission = absence" below are now cashed in as two minimal additive fields (`versionGroup`, `excluded`). This ADR's core still holds: every version is its own distinct Entry, surfaced for review, never silently merged or dropped.
