...
**Direct-interaction override** (fail-safe):
The documented escape hatch for when a maintainer's ask can't be expressed in the **Curation wizard** — a global/relational ordering rule, a bulk edit across many Entries, or a constraint spanning Entries a **Lens** can't reach. The maintainer drops to a conversational/deterministic channel: edits the **Draft** directly (or a focused subagent computes the change and the assistant audits it), then **reloads the wizard to rejoin** and publishes through the same **Publish gate**. The override only ever touches the Draft — never `data.json` — so it is *more* control, never *less* validation. The wizard stays the default surface; the override is an offered detour, never a silent bypass. See [[0016-direct-interaction-override-failsafe]].
_Avoid_: using "override" for a theme one-off (that's a **Layout Mode**/`theme.css` override) or for editing the published file (the override edits the **Draft**, then re-enters the gate).
